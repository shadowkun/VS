...
namespace shadow
{
    ...
    struct CharTraits < char > : public std::char_traits < char >
    {
        ...
        static char_type* tstrchr(const char_type *s, char_type c)							{ return ::strchr(s, c); }
        ...
        static char_type* tstrstr(const char_type *s, const char_type *sub)					{ return ::strstr(s, sub); }
    ...
    struct CharTraits < wchar_t > : public std::char_traits < wchar_t >
    {
        ...
        static char_type* tstrchr(const char_type *s, char_type c)							{ return ::wcschr(s, c); }
        ...
        static char_type* tstrstr(const char_type *s, const char_type *sub)					{ return ::wcsstr(s, sub); }
    ...
    inline int CompareNoCase(const Ch* s1, const Ch* s2)
    {
        ...
        /*
         for(; *s1 != 0 && *s2 != 0; ++s1, ++s2){
         // compare character
         int r = CharTraits<Ch>::ttolower(*s1)
         - CharTraits<Ch>::ttolower(*s2);
         if(r > 0)
         return 1;
         if(r < 0)
         return -1;
    ...
    inline void ReplaceAllCopy(II_S fs, II_S ls,
                               II_O fo,
                               II_O lo,
                               II_N fn,
                               II_N ln,
    ...
    bool MatchWildcard(FI1 f1, FI1 l1, FI2 f2, FI2 l2, ValueT asterisk, ValueT question)
    {
        for(; f1 != l1 && f2 != l2 ; ){
            char c1 = *f1, c2 = *f2;
            if(c1 == asterisk){
                ++f1;
                while(f2 != l2){
                    if(MatchWildcard(f1, l1, f2, l2, asterisk, question))
                        return true;
                    ++f2;
                }
                return f1 == l1;
    ...
    inline bool Split(II_S fs, II_S ls,
                      II_O fo, II_O lo,
                      Pr pr,
                      OI o,
                      const StringT * x)
    ...
    inline std::basic_string<typename std::iterator_traits<FI2>::value_type> Join(FI1 f1, FI1 l1, FI2 f2, FI2 l2)
    {
    ...
    {
        return Join(f1, l1, separator.begin(), separator.end());
    }
    ...
    {
        return Join(f1, l1, separator, CharTraits<Ch>::tstrend(separator));
    }
    ...
    {
        return Join(f1, l1, &separator, &separator + 1);
    }
